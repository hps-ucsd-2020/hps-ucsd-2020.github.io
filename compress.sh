...
# Step 4: Create a copy of the image in the folder called cropped. If there is no such folder, create a cropped folder
# Step 5: Folder structure should resemble folder structure of assets/img/slideshow-images/dive-img/
# Step 6: Crop the image to have a 4:3 size
# Step 7: Run the program on the folder containing the images
# Step 8: ???
# Step 9: Profit (The program will compress the cropped images and put them in the parent folder of the cropped images ready for use with the website)

if [ $# == 2 ] && [ -d "$1" ]
then
  if [ "$2" -ge 100 ] && [ "$2" -le 2000 ]
  then
    folders=$(find "$1" -type d -name "cropped")
    for i in $folders
    do
